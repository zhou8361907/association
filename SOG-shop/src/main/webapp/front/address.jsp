			...
			<div class="section"> 
					<div id="shop2" class="product_frame">					
							...
							<div class="div_two" style="position: absolute;">
								<h3  class="c_money_one">￥${choose.getGood().getGood_sale_price() }</h3>
								<lable >${choose.getNumber() }</lable>
								<h3 style="margin-left:6.8rem" class="c_all_money">￥${choose.getGood().getGood_sale_price()*choose.getNumber() }</h3>
							</div>
					</div>
			</div>
			</c:forEach>
			
			<span style="margin-left: 62%; font-size: 20px;">商品总价为:￥</span><span style="font-size: 20px;">${money_all }</span>
			<input type="hidden" name="choose_list" value="${check_list }"/>
