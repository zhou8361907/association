								...
								<table class="table table-striped table-bordered table-hover" id="dataTables-example">
									...
									<c:forEach items="${map.list}" var="order">
										<tr class="odd gradeX">
											...
											<td class="center">已退货</td>
											<td class="center">${order.sumMoney }</td>
											<td class="center">${order.beginTime }</td>
											<td class="center">${order.finishTime}</td>
											<td>
											<form action="/admin/OrdersServlet?id=${order.orderId}&flag=edit&cur=${curPage}&selectid=1" method="post">
											<select name="change">
								...
								<div class="col-md-6" style="text-align: right;">
									...
									<c:forEach var="i" begin="${begin }" end="${end }">
										<c:choose>
											...
											<c:otherwise>
												<li><a href="/admin/OrdersServlet?selectid=1&curPage=${i}">${i}</a></li>
