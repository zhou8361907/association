...
%>
<%@page import="as.event,as.student,as.asso" %>
...
<div class="container">
	<div class="row clearfix">
		<div class="col-md-12 column">
		<h1>社团搜索</h1>
		 <form name="form" action="servlet/as_search" method="post" >
			<ul class="breadcrumb">
				<li>
					 <p>类型：</p>
				</li>
				<li>
					 	<input type="radio" name="a_new" value="-1" required>全部</input>
				</li>
				<li>
						<input type="radio" name="a_new" value="1" required>新兴社团</input>
				</li>
				<li>
					<input type="radio" name="a_new" value="0" required>热门社团</input>
				</li>
			</ul>
		...
		list=(List)request.getAttribute("as_search");
		%>
		<table border="1">
			<%
			for(asso a:list)
			{
			 %>
			<tr>
				 <a href="servlet/as_put?id=<%=a.getA_id() %>" class="list-group-item active"><%=a.getA_name() %></a>
				<div class="list-group-item">
				团长：  <%=a.getA_head() %> 
				&nbsp;&nbsp;&nbsp;&nbsp;  成立时间： <%=a.getA_time() %> 
				&nbsp;&nbsp;&nbsp;&nbsp;  类型： <%if(a.getA_new()==0){ %><h>热门社团<h><%  }else  {%><h>新兴社团</h> <% }%>
				&nbsp;&nbsp;&nbsp;&nbsp;  关注数：<%=a.getA_follow() %>
				</div>
				</tr>
			<%} %>
